...
func main() {
	/*
		...
		i value, which will be updated each time we
		call nextInt.
	*/
	nextInt := intSeq()

	// see the effect of the closure by calling it a few times
	fmt.Println(nextInt())
	fmt.Println(nextInt())
	fmt.Println(nextInt())

	/*
		To confirm that the stat is unique to that particular
...
/* The last feature of functions we’ll look at
for now is recursion.
*/
